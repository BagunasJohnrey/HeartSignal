...
const UserDot = ({ user, onPress, isSignalsMode }: { user: any, onPress: (u: any) => void, isSignalsMode: boolean }) => {
  ...
  React.useEffect(() => { opacity.value = withTiming(isSignalsMode ? 0 : 1, { duration: 500 }); }, [isSignalsMode]);
  React.useEffect(() => { translateY.value = withRepeat(withTiming(user.id % 2 === 0 ? 5 : -5, { duration: 2000 + (user.id * 100), easing: Easing.inOut(Easing.ease) }), -1, true); }, []);
